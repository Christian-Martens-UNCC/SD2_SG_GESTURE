...
main_layout = [
    [sg.Column([
        ...
        sg.Column([
            ...
            [sg.Text('Step 1: Perform roundness measurement with 5 needles', justification='l', font='_ 20',
                     expand_x=True)],
            [sg.Text('Step 1a: Show the adjustment gesture if an adjustment was made', justification='l', font='_ 18',
                     expand_x=True)],
            [sg.Text('Step 2: Perform inner diameter measurement with 5 needles', justification='l', font='_ 20',
                     expand_x=True)],
            [sg.Text('Step 2a: Show the adjustment gesture if an adjustment was made', justification='l', font='_ 18',
                     expand_x=True)],
            [sg.Text('Step 3: Perform outer diameter measurement with 5 needles', justification='l', font='_ 20',
                     expand_x=True)],
            [sg.Text('Step 3a: Show the adjustment gesture if an adjustment was made', justification='l', font='_ 18',
                     expand_x=True)],
            [sg.Text('', justification='center', expand_x=True)],
            [sg.Text('', justification='center', expand_x=True)],
...
if nano_cam:
    ...
    cap = cv2.VideoCapture(
        'nvarguscamerasrc sensor-mode=4 ! video/x-raw(memory:NVMM), width=(int)640, height=(int)480,'
        ' format=(string)NV12, '
        'framerate=(fraction)20/1 ! nvvidconv flip-method=2 ! videobalance contrast=1.0 brightness=0.15 ! video/x-raw, '
        'format=(string)BGRx ! videoconvert ! queue max-size-buffers=1 leaky=downstream ! video/x-raw, '
...
while True:
    ...
    if boot_up:
        ...
        i_gray_blur = cv2.GaussianBlur(i_gray, (7, 7), cv2.BORDER_DEFAULT)
        i_blur = cv2.GaussianBlur(frame, (21, 21), cv2.BORDER_DEFAULT)
        boot_up = False
    h, w, c = frame.shape
    x_pos, y_pos = [], []
    n = []
    ...
    img_gray_blur = cv2.GaussianBlur(img_gray, (7, 7), cv2.BORDER_DEFAULT)

    img_diff = cv2.subtract(i_gray_blur, img_gray_blur)
    img_contour = img_black.copy()

    _, mask = cv2.threshold(img_diff, 127, 255, cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)
    img_diff[mask != 255] = [255]
    img_diff[mask == 255] = [0]

    _, thresh = cv2.threshold(img_diff, 127, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
    contours, _ = cv2.findContours(thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
    cv2.drawContours(img_contour, contours, -1, (255, 255, 255), 2)

    inv_img_contour = cv2.bitwise_not(img_contour)
    mask_h, mask_w = inv_img_contour.shape[:2]
    mask = cv2.resize(img_black, [mask_w+2, mask_h+2])
    cv2.floodFill(inv_img_contour, mask, (10, 10), 0);
    complete_mask = inv_img_contour | img_diff | img_contour

    mask_color = cv2.bitwise_and(image_copy, image_copy, mask=complete_mask)
    image_pro = concat_img = cv2.addWeighted(i_blur, 0.2, mask_color, 0.8, 0)

    results = hands.process(image_pro)
    ...
    if run_model:
        ...
        display_img = resize_image(image_pro, 170)
